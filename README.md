...
**SelfLift 统一续接：** 所有段长均按实际窗口采样，与动作续接一样固定继承22帧上下文。不额外生成未来部分，也不将继承区额外添加为历史视频条件。比如首段交付73帧，后续各交付51帧，三轮续接共226帧。
...
续接固定使用 **22帧原生上下文（7个latent时间步）**。首段不接 `previous_latent`，条件与latent直接透传；后续段必须接前段同分辨率的音视频latent。

低清续接接前段低清采样器的去噪输出，高清续接接前段高清采样结果。循环内两路latent必须分别回传，不能只建立执行依赖。单张首帧条件转为身份参考；上下文直接复制，不反复编码图片。

`boundary_check` 开启时连接 `video_vae`；低清、高清两处的 `previous_frames` 都接上一段最终高清画面，不要预先缩小。节点通过图片和latent的尺寸识别低清阶段：只有低清继承区参与校准，同分辨率高清上下文保持原始值。关闭时不调用VAE，也不执行校准。普通动作续接没有接图片时只能检查自身latent，无法据此恢复最终高清背景。
...
H3本身使用17帧周期，完整片段为 `17k+5` 帧。**124帧目标：首段交付124帧，续段内部141帧，裁掉22帧后新增119帧。** 后续音频起点必须按实际交付长度推进。
