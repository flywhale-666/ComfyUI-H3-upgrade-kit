...
H3KIT_LAYOUT_LOG = logging.getLogger("h3kit_motion")

_motion_layout_status = None  # None not yet run, True passed, str the failure


class H3KitLayoutShape:
...
def layout_segments(layout, kind):
    return [(a, b) for a, b, k in layout.segments if k == kind]


def verify_motion_coordinates():
    ...
    video = H3KitLayoutShape((1, 24, 1, lh, lw))
    ...
    lay = build(keyframes=[{"resolved_frame_index": p, "latent": video}
                           for p in (0, 3)])
    o = timeline_origin(lay)
    got = [float(lay.position_ids[a, 0]) - o for a, _ in layout_segments(lay, "cond")]
    want = [0.0, fr * 3]
    ...
    a, _ = layout_segments(lay, "cond")[0]
    gap = float(lay.position_ids[a, 0]) - timeline_origin(lay)
    ...
    lay = build(keyframes=[{"resolved_frame_index": idx,
                            "audio_latent": H3KitLayoutShape((1, 32, 2, rt))}])
    spans = layout_segments(lay, "cond_audio")
    if len(spans) != 1:
        raise RuntimeError(
...
def ensure_motion_layout(context=""):
    """Run the checks once. Raises with a usable message if they fail."""
    global _motion_layout_status
    if _motion_layout_status is True:
        return
    if isinstance(_motion_layout_status, str):
        raise RuntimeError(_motion_layout_status)

    if not hasattr(mm, "PackedLayout") or not hasattr(mm, "FRAME_RESCALE"):
        _motion_layout_status = ("h3kit_motion: ComfyUI's MiniMax H3 model module is "
    ...
    try:
        ...
        params = {}
    wrapped = layout_wrapper_origin()
    if wrapped:
        H3KIT_LAYOUT_LOG.warning(
            "h3kit_motion: H3's layout constructor has been wrapped by "
    ...
    try:
        ...
        raise RuntimeError(_motion_layout_status) from exc

    _motion_layout_status = True
    H3KIT_LAYOUT_LOG.info("h3kit_motion: ComfyUI H3 layout checks passed, anchors "
